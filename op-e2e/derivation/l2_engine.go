...
func computePayloadId(headBlockHash common.Hash, params *eth.PayloadAttributes) beacon.PayloadID {
	...
	for _, tx := range params.Transactions {
		binary.Write(hasher, binary.BigEndian, uint64(len(tx))) // length-prefix to avoid collisions
		hasher.Write(tx)
	}
	if params.NoTxPool {
		hasher.Write([]byte{1})
...
func (e *L2EngineAPI) GetPayload(ctx context.Context, payloadId eth.PayloadID) (*eth.ExecutionPayload, error) {
...
func (e *L2EngineAPI) ForkchoiceUpdate(ctx context.Context, state *eth.ForkchoiceState, attr *eth.PayloadAttributes) (*eth.ForkchoiceUpdatedResult, error) {
...
func (e *L2EngineAPI) NewPayload(ctx context.Context, payload *eth.ExecutionPayload) (*eth.PayloadStatusV1, error) {
...
func (s *L2Engine) actL2RPCFail(ctx context.Context) error {
...
func (s *L2Engine) actL2IncludeTx(ctx context.Context) error {
	if s.l2BuildingHeader == nil {
		return fmt.Errorf("not currently building a block, cannot include tx from queue: %w", InvalidActionErr)
	}
	if len(s.l2TxQueue) == 0 {
		return fmt.Errorf("tx queue is empty: %w", InvalidActionErr)
	}
	if s.l2ForceEmpty {
		return fmt.Errorf("cannot include any sequencer txs: %w", InvalidActionErr)
	}
